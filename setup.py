...
def _get_zond2_spec_comment(child: LinkRefDef) -> Optional[str]:
    _, _, title = child._parse_info
    if not (title[0] == "(" and title[len(title)-1] == ")"):
        return None
    title = title[1:len(title)-1]
    if not title.startswith(ZOND2_SPEC_COMMENT_PREFIX):
        return None
    return title[len(ZOND2_SPEC_COMMENT_PREFIX):].strip()
...
def get_spec(file_name: Path, preset: Dict[str, str], config: Dict[str, str], preset_name=str) -> SpecObject:
    ...
    for child in document.children:
        ...
        if isinstance(child, Heading):
            ...
            comment = _get_zond2_spec_comment(child)
            if comment == "skip":
                should_skip = True
...
class BuildPyCommand(build_py):
    ...
    def run_pyspec_cmd(self, spec_fork: str, **opts):
        ...
        cmd_obj.out_dir = os.path.join(self.build_lib, 'zond2spec', spec_fork)
...
class PyspecDevCommand(Command):
    ...
    def run_pyspec_cmd(self, spec_fork: str, **opts):
        cmd_obj: PySpecCommand = self.distribution.reinitialize_command("pyspec")
        cmd_obj.spec_fork = spec_fork
        zond2spec_dir = convert_path(self.distribution.package_dir['zond2spec'])
        cmd_obj.out_dir = os.path.join(zond2spec_dir, spec_fork)
        for k, v in opts.items():
            setattr(cmd_obj, k, v)
        self.run_command('pyspec')
...
with open(os.path.join('tests', 'core', 'pyspec', 'zond2spec', 'VERSION.txt')) as f:
    spec_version = f.read().strip()

setup(
    name='zond2spec',
    version=spec_version,
    description="Zond2 spec, provided as Python package for tooling and testing",
    ...
    package_data={'configs': ['*.yaml'],
                  'presets': ['*.yaml'],
                  'specs': ['**/*.md'],
                  'zond2spec': ['VERSION.txt']},
    package_dir={
        "zond2spec": "tests/core/pyspec/zond2spec",
        "configs": "configs",
        "presets": "presets",
        "specs": "specs",
    },
    packages=find_packages(where='tests/core/pyspec') + ['configs', 'specs'],
    py_modules=["zond2spec"],
    cmdclass=commands,
