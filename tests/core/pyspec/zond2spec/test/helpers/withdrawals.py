...
def set_validator_fully_withdrawable(spec, state, index, withdrawable_epoch=None):
    ...
    if validator.withdrawal_credentials[0:1] == spec.BLS_WITHDRAWAL_PREFIX:
        validator.withdrawal_credentials = spec.ZOND1_ADDRESS_WITHDRAWAL_PREFIX + validator.withdrawal_credentials[1:]
...
def set_zond1_withdrawal_credential_with_balance(spec, state, index, balance=None, address=None):
    if balance is None:
    ...
    validator = state.validators[index]
    validator.withdrawal_credentials = spec.ZOND1_ADDRESS_WITHDRAWAL_PREFIX + b'\x00' * 11 + address
...
def set_validator_partially_withdrawable(spec, state, index, excess_balance=1000000000):
    set_zond1_withdrawal_credential_with_balance(spec, state, index, spec.MAX_EFFECTIVE_BALANCE + excess_balance)
    validator = state.validators[index]
