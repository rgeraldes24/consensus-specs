...
from zond2spec.test.helpers.attestations import next_slots_with_attestations
...
from zond2spec.test.helpers.deposits import (
    prepare_state_and_deposit,
)
from zond2spec.test.helpers.proposer_slashings import (
...
from zond2spec.test.helpers.voluntary_exits import (
...
def _state_transition_and_sign_block_at_slot(spec,
    ...
    p.s. we can't just pass `body` and assign it because randao_reveal and zond1_data was set in `build_empty_block`
...
def run_transition_with_operation(state,
    ...
    def _check_state():
        if operation_type == OperationType.PROPOSER_SLASHING:
            ...
            validator = state.validators[selected_validator_index]
            assert validator.withdrawal_credentials[:1] == spec.ZOND1_ADDRESS_WITHDRAWAL_PREFIX
