from zond2spec.test.context import spec_state_test, always_bls, with_all_phases
...
@with_all_phases
@spec_state_test
def test_new_deposit_zond1_withdrawal_credentials(spec, state):
    # fresh deposit = next validator index = validator appended to registry
    validator_index = len(state.validators)
    withdrawal_credentials = (
        spec.ZOND1_ADDRESS_WITHDRAWAL_PREFIX
        + b'\x00' * 11  # specified 0s
        + b'\x59' * 20  # a 20-byte zond1 address
    )
    amount = spec.MAX_EFFECTIVE_BALANCE
...
def test_invalid_wrong_deposit_for_deposit_count(spec, state):
    ...
    state.zond1_data.deposit_root = root_2
    state.zond1_data.deposit_count = deposit_count_1
