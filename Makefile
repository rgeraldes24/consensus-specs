...
ZOND2SPEC_MODULE_DIR = $(PY_SPEC_DIR)/zond2spec
...
ALL_EXECUTABLE_SPEC_NAMES = capella
# The parameters for commands. Use `foreach` to avoid listing specs again.
COVERAGE_SCOPE := $(foreach S,$(ALL_EXECUTABLE_SPEC_NAMES), --cov=zond2spec.$S.$(TEST_PRESET_TYPE))
PYLINT_SCOPE := $(foreach S,$(ALL_EXECUTABLE_SPEC_NAMES), ./zond2spec/$S)
MYPY_SCOPE := $(foreach S,$(ALL_EXECUTABLE_SPEC_NAMES), -p zond2spec.$S)
...
partial_clean:
	...
	rm -rf zond2spec.egg-info dist build
	rm -rf build;
	@for spec_name in $(ALL_EXECUTABLE_SPEC_NAMES) ; do \
		echo $$spec_name; \
		rm -rf $(ZOND2SPEC_MODULE_DIR)/$$spec_name; \
	done
...
# The pyspec is rebuilt to enforce the /specs being part of zond2specs source distribution. It could be forgotten otherwise.
dist_build: pyspec
...
test: pyspec
	...
	python3 -m pytest -n 4 --disable-bls $(COVERAGE_SCOPE) --cov-report="html:$(COV_HTML_OUT)" --cov-branch zond2spec

# Testing against `minimal` or `mainnet` config by default
find_test: pyspec
	. venv/bin/activate; cd $(PY_SPEC_DIR); \
	python3 -m pytest -k=$(K) --disable-bls $(COVERAGE_SCOPE) --cov-report="html:$(COV_HTML_OUT)" --cov-branch zond2spec

citest: pyspec
	mkdir -p $(TEST_REPORT_DIR);
ifdef fork
	. venv/bin/activate; cd $(PY_SPEC_DIR); \
	python3 -m pytest -n 16 --preset=$(TEST_PRESET_TYPE) --fork=$(fork) --junitxml=test-reports/test_results.xml zond2spec
else
	. venv/bin/activate; cd $(PY_SPEC_DIR); \
	python3 -m pytest -n 16 --preset=$(TEST_PRESET_TYPE) --junitxml=test-reports/test_results.xml zond2spec
endif
...
lint: pyspec
	. venv/bin/activate; cd $(PY_SPEC_DIR); \
	flake8  --config $(LINTER_CONFIG_FILE) ./zond2spec \
